...
export default {
	...
	mounted() {
		this.initWebUpload();
		var that = this;
		// 刷新页面时取消上传，同时删除服务端所有chunks文件
		window.onbeforeunload = function() {
			var progressList = that.getFiles("progress");
	...
	methods: {
		initWebUpload() {
			...
			const fun = ["fileQueued", "fileDequeued", "uploadStart", "uploadProgress", "uploadSuccess", "uploadError", "error", "uploadFinished"];
		...
		error(type) {
			...
			if (type == "Q_EXCEED_NUM_LIMIT") {
				...
				msg = "添加的文件总大小超过" + WebUploader.Base.formatSize(this.fileSizeLimit);
			} else if (type == "F_EXCEED_SIZE") {
				msg = "添加的文件大小超过" + WebUploader.Base.formatSize(this.fileSingleSizeLimit);
			} else {
				msg = type;
			}
