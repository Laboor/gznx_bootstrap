<template>
	<div class="home-page">
		...
		<div class="main-content">
			...
			>
				<el-select v-model="commandType" slot="prepend" @change="handleCommandChange" @click.native="$refs.autocomplete.close()">
					<el-option class="el-option" v-for="opt of commandTypeOpts" :key="opt.value" :label="opt.label" :value="opt.value"> </el-option>
				</el-select>
				<el-button class="input-append-btn" slot="append" type="primary" @click="handleExec">{{ commandTypeOpt.execBtnText }}</el-button>
			</el-autocomplete>

			<input ref="shadowFileInput" v-if="commandType == '3'" type="file" style="display: none" @change="handleAddFile" />

			<div class="main-table">
				<el-table ref="dataTable" :data="execTableData" v-show="commandType != '3'" :height="dataTableHeight">
					<el-table-column prop="command" label="命令" width="350"></el-table-column>
					<el-table-column prop="type" label="类型">
						<template slot-scope="scope">
							<el-tag v-if="scope.row.type == 'shell'" style="border-color: #d3adf7; color: #915de7;" color="#f9f0ff" type="success" size="small"
								>脚本</el-tag
							>
							<el-tag v-if="scope.row.type == 'log'" style="border-color: #87e8de; color: #08979c;" color="#e6fffb" type="success" size="small"
								>日志</el-tag
							>
						</template>
					</el-table-column>
					...
					<el-table-column prop="option" label="操作">
						<template slot-scope="scope">
							<el-button size="mini" type="danger" @click="handleInterruptCommand(scope, scope.row)">中断</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-table v-show="commandType == '3'" :data="uploadTableData" :height="dataTableHeight">
					...
					<el-table-column prop="file.uploadDir" label="上传路径" width="300">
						<template slot-scope="scope">
							<el-input
								...
								placeholder="请输入上传路径"
							>
								<el-button class="input-mini-btn" slot="append" icon="el-icon-check" @click="handleConfirmUploadPath(scope, scope.row)"></el-button>
							</el-input>
							<template v-else>
								{{ scope.row.file.uploadDir }}
								<el-tooltip v-if="!scope.row.isStart" content="修改" :hide-after="2000" :open-delay="500" placement="top">
									<i class="el-icon-edit edit-upload-path-icon" @click="handleEditUploadPath(scope, scope.row)"></i>
								</el-tooltip>
							</template>
						</template>
					</el-table-column>
					<el-table-column prop="progressBar" label="上传进度">
						<template slot-scope="scope">
							...
							<template v-if="!scope.row.success && !scope.row.error">
								<el-tooltip v-if="!scope.row.pause" content="暂停" :hide-after="2000" :open-delay="500" placement="top">
									<i
								...
								<el-tooltip v-else content="继续" :hide-after="2000" :open-delay="500" placement="top">
									<i class="el-icon-video-play upload-option-btn" style="color: #42b983" @click="handlePauseUpload(scope, scope.row)"></i>
								</el-tooltip>
								<el-tooltip content="取消" :hide-after="2000" :open-delay="500" placement="top">
									<i class="el-icon-error upload-option-btn" style="color: #ff4d4f" @click="handleCancelUpload(scope, scope.row)"></i>
								</el-tooltip>
							</template>
							<template v-if="scope.row.error">
								<el-tooltip :content="'文件上传失败，错误信息：' + scope.row.errorMsg" :hide-after="3000" :open-delay="500" placement="top">
									<i class="el-icon-warning upload-error"></i>
								</el-tooltip>
								<el-tooltip content="重试" :hide-after="2000" :open-delay="500" placement="top">
									<i class="el-icon-refresh-right upload-retry" @click="handleRetryUpload(scope, scope.row)"></i>
								</el-tooltip>
							</template>
							<span v-if="scope.row.success"> <i class="el-icon-success upload-success" />上传完成 </span>
						</template>
					</el-table-column>
				</el-table>
			</div>
		...
		<el-dialog title="确认信息" :visible.sync="confirmInfoDialog" width="500px" center @closed="isPrintLog = true">
			<span class="dialog-label">操作类型：</span
			><el-tag size="medium" style="border-color: #d3adf7; color: #915de7;" color="#f9f0ff">{{ commandTypeOpt.label }}</el-tag
			><br />
			<span class="dialog-label">文件路径：</span>
			<el-tooltip v-if="commandTag" class="item" effect="dark" :content="command" placement="top" :open-delay="1000">
				<el-tag size="medium" style="border-color: #d3adf7; color: #915de7;" color="#f9f0ff">{{ commandTag }}</el-tag>
			</el-tooltip>
			<el-tag v-else size="medium" style="border-color: #d3adf7; color: #915de7;" color="#f9f0ff">{{ command }}</el-tag>
			<br />
			<span class="dialog-label">是否打印日志：</span>
			<el-radio class="dialog-content" v-model="isPrintLog" :label="true">是</el-radio>
			<el-radio class="dialog-content" v-model="isPrintLog" :label="false">否</el-radio><br />
			<template v-if="isPrintLog">
				<span class="dialog-label">字符集：</span>
				<el-radio class="dialog-content" v-model="logCharset" label="UTF-8">UTF-8</el-radio>
				<el-radio class="dialog-content" v-model="logCharset" label="GBK">GBK</el-radio>
			</template>
			<span slot="footer">
...
export default {
	...
	methods: {
		...
		getHistoryListFromLocalStorage() {
			var historyList = window.localStorage.getItem("execHistoryList");
		...
		addHistoryList(command) {
			...
			window.localStorage.setItem("execHistoryList", newHistoryList);
		},
		execCommand() {
			return this.axios
				...
				.then((res) => {
					if (res.data.success) {
						// 加入历史执行列表
						this.addHistoryList(this.command);
						return res.data;
					} else {
