<template>
	<div class="log-view">
		<div class="tool-bar">
			<el-tooltip v-if="printPause" content="继续" :hide-after="2000" :open-delay="500">
				<i class="el-icon-video-play tool-btn" style="color: #42b983" @click="handlePlay" />
			</el-tooltip>
			<el-tooltip v-else content="暂停" :hide-after="2000" :open-delay="500">
				<i class="el-icon-video-pause tool-btn" style="color: red" @click="handlePlay" />
			</el-tooltip>
			<el-tooltip content="下载" :hide-after="2000" :open-delay="500">
				<i class="el-icon-download tool-btn tool-btn-hover-blue" @click="handleDownload" />
			</el-tooltip>
			<el-tooltip content="设置" :hide-after="2000" :open-delay="500">
				<i class="el-icon-setting tool-btn tool-btn-hover-blue" @click="handleSetting" />
			</el-tooltip>
			<el-tooltip content="清空" :hide-after="2000" :open-delay="500">
				<i class="el-icon-delete tool-btn tool-btn-hover-red" @click="handleClearLog" />
			</el-tooltip>
			<el-divider class="tool-divider" direction="vertical" />
			<el-tooltip content="关闭" :hide-after="2000" :open-delay="500">
				<i class="el-icon-circle-close tool-btn tool-btn-hover-red" @click="handleCloseWin" />
			</el-tooltip>
		</div>
		<el-dialog title="设置主题" :visible.sync="themeSettingDialog" width="300px">
			<el-radio v-model="theme" label="light">白色</el-radio>
			<el-radio v-model="theme" label="dark">黑色</el-radio>
		</el-dialog>
		<log-message ref="logMessage" :url="websocketUrl" :pause="printPause" :theme="theme" @connected="handleConnected" />
	</div>
</template>
...
export default {
	...
	created() {
		var queryStr = "?type=" + this.$route.query.type + "&command=" + this.$route.query.command;
